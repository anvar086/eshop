<main id="main">
    <div class="container">
        ...
        <!--On Sale-->
        @if($sproducts->count()>0 && $sale->status==1 && $sale->sale > Carbon\Carbon::now())
        <div class="wrap-show-advance-info-box style-1 has-countdown">
            <h3 class="title-box">On Sale</h3>
            <div class="wrap-countdown mercado-countdown" data-expire="{{Carbon\Carbon::parse($sale->sale)->format('Y/m/d h:m:s')}}"></div>
                ...
                <div class="wrap-product-tab tab-style-1">
                    <div class="tab-contents">
                        <div class="tab-content-item active" id="digital_1a">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                            @foreach($late_products as $late_product)
                                <div class="product product-style-2 equal-elem ">
                                    <div class="product-thumnail">
                                        <a href="{{route('product.details',['slug'=>$late_product->slug])}}" title="{{$late_product->name}}">
                                            <figure><img src="{{asset('assets/images/products')}}/{{$late_product->image}}" width="800" height="800" alt="{{$late_product->name}}"></figure>
                                        </a>
                                    </div>
                                    <div class="product-info">
                                        <a href="{{route('product.details',['slug'=>$late_product->slug])}}" class="product-name"><span>{{$late_product->name}}</span></a>
                                        <div class="wrap-price"><span class="product-price">${{$late_product->regular_price}}</span></div>
                                    </div>
                                </div>
                            @endforeach
                            </div>
